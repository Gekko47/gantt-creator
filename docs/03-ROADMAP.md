...
| R2.7c | Carry each named style's resolved formatting and default label position in the Core style registry so resolution covers user styles, not only built-in presets | Registry-construction guard tests, built-in and user-style projection tests, resolver override-precedence and Type-default-fallback tests, and a contract round-trip proving a user style's formatting survives save/reopen | None |
