...
- **Decision** (human-approved 2026-09-21):
  ...
  - **D7 — Table geometry and `tblGanttStyles` column layout.** Tables are
    ...
    `MilestoneSizePt`, `DefaultLabelPosition`, `AllowedLabelPositions`, `ColourCapability`. **ADR-0009 (2026-09-23) defines the final 14-column `tblGanttStyles` layout so Custom Activity and R3 style resolution have explicit named-style capabilities.** `tblGanttTypes` columns: `TypeName`, `DisplayName`,
