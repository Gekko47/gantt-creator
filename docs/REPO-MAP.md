...
| [Core](../src/GanttCreator.Core/) | PointD, VersionInfo, Logging, workbook schema contract (GanttTableColumns/GanttSchemaVersion, GanttWorkbookContract), the code-owned EntityTypeCatalog, stable row identity (GanttRowId), and DTO-to-event all-errors validation (GanttRowValidator/GanttEvent); no Office or UI dependency |
