...
- **R0.5 — Sample tests, OfficeIntegration trait, `.vscode/tasks.json`** — 6 `UnitTest1.cs` placeholders deleted; 6 sample test classes added (one per test project); 2 `[Trait("Category","OfficeIntegration")]` tests in `Office.IntegrationTests` (Excel + PowerPoint COM progid presence). `.vscode/tasks.json` with 6 tasks mirroring the verify scripts. `NoWarn` tightened from 8 to 1 (only `CA1707` remains, with documented rationale). `GenerateDocumentationFile=true` in production so CS1591 enforces XML doc comments; test projects override to `false` because tests aren't API. Production `ExportSize` type lives in `GanttCreator.Raster` (R8.1 seed); culture-invariant parsing, tuple-switch validation, brace-styled ifs. `dotnet test --filter "Category=OfficeIntegration"` → 2/2 PASS. Full suite: 35 tests across 6 projects. Commits `eba9339` (initial), `12e8329` (analyzer fixes), `6578155` (XML doc enforcement). `pwsh ./scripts/verify-quick.ps1` → PASS in ~11 s.
