...
- **Next safe action**: R0.7 — Windows CI workflow (`.github/workflows/ci.yml`); branch protection on `main`; required status check is `pwsh ./scripts/verify.ps1`; Office-integration runs only on a self-hosted runner.
