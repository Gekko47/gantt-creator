...
| L9 | Pester is not pinned to one version: the dev machine has 6.1.0, CI installs newest ≥ 5.0, and the two majors differ in `-PassThru` result shape (`.TestResult` vs container counts). `scripts/test-scripts.ps1` now uses version-safe counts, but exact version pinning and single-sourcing (Pester, PSScriptAnalyzer, actionlint) are deferred | _unassigned_ | 2026-09-07 | W11 | Work item W11: one tool-version source consumed by `ci.yml` and `scripts/`, with an anti-drift Pester test |
